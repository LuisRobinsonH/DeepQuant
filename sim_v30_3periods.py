...
# === HEADER ===
print("=" * 80)
print("  DEEPQUANT V30 — Trade Quality: rs_vs_VAS feature + EMA5 + ATR compression (V29 base)")
print(f"  RISK_GUARD={RISK_GUARD_PCT*100:.0f}% | PROB_SIZE={PROB_SIZE_MIN*100:.0f}-{PROB_SIZE_MAX*100:.0f}% | MAX_POS={MAX_POS}")
print(f"  NEW: rs_vs_vas_10 ML feature | F6: close>=EMA5 | F7: ATR(7)/ATR(28)<1.25")
print("=" * 80)
print(f"  BULL:     +{BE_TRIGGER_ATR}xATR/-{SL_ATR}xATR {MAX_HOLD}d | 3m+ | prob>=0.52/0.58 | prob-scaled vs capital")
...
def simulate_period(name, start, end, train_end, capital):
    ...
    for t in all_model_tickers:
        ...
        atr7  = volatility.average_true_range(df["High"], df["Low"], df["Close"], 7)   # V30
        atr28 = volatility.average_true_range(df["High"], df["Low"], df["Close"], 28)  # V30
        ema5  = df["Close"].ewm(span=5, adjust=False).mean()                            # V30
        ...
        price_cache[t] = pd.DataFrame({
            "atr": atr, "atr7": atr7, "atr28": atr28, "ema5": ema5,  # V30 additions
    ...
    for day in trading_days:
        ...
        for t in tier_tickers:
            ...
            # F6 (V30): Price above EMA5 — no counter-trend/pullback entries
            ema5_val = p_row.get("ema5")
            if pd.notna(ema5_val) and ema5_val > 0 and price < ema5_val * 0.997:
                continue

            # F7 (V30): ATR compression — avoid choppy expanding-volatility entries
            atr7_val  = p_row.get("atr7")
            atr28_val = p_row.get("atr28")
            if pd.notna(atr7_val) and pd.notna(atr28_val) and atr28_val > 0:
                if atr7_val / atr28_val > 1.25:
                    continue
...
if __name__ == "__main__":
    # ═══════════════════════════════════════════════════════════════
    # DEEPQUANT V30 — IMPROVEMENT NOTES & RECOMMENDATIONS
    # ═══════════════════════════════════════════════════════════════
    #
    ...
    #  Per-period summary 
    print(f"\n\n{'='*80}")
    print(f"  SUMMARY - DEEPQUANT V30 (V29 base: rs_vs_VAS + EMA5 + ATR compression)")
    print(f"{'='*80}")
    ...
    print(f"  V24   +2.22%  -11.38%   +0.00%   -9.41%      $0   $-283   $+461   $-910      $0  3/5 +dist_52w_high FAILED")
    print(f"  V25  +21.46%   +0.32%   +0.00%  +21.85%      $0    $+44 $+1,673    $+25      $0  5/5 prob>=0.70 (lower cap)")
    print(f"  V26  +25.09%   +0.80%   +0.00%  +26.09%      $0    $+44 $+1,964    $+64      $0  5/5 RSI<=78 == V22b")
    print(f"  V28    +6.90%   +6.53%   +0.00%  +13.88%      $0    -$3   $+555   $+522      $0  4/5 RISK3/SIZE40/POS2")
    print(f"  V28b  +11.38%   +1.27%   +0.00%  +12.80%      $0    $+11   $+899   $+101      $0  5/5 RISK5/SIZE55/POS2")
    print(f"  V29   +18.61%   +0.49%   +0.00%  +19.19%      $0    $+30 $+1,459    $+39      $0  5/5 RISK7/SIZE75/POS1")
    print(f"  V30  {p1['roi']:>+7.2f}% {p2['roi']:>+7.2f}% {p3['roi']:>+7.2f}% {cum_roi:>+7.2f}% {yr_all['2022']:>7s} {yr_all['2023']:>7s} {yr_all['2024']:>7s} {yr_all['2025']:>7s} {yr_all['2026']:>7s}  {passed}/{passed+failed} rs_vas+EMA5+ATR")
    print(f"{'='*90}")
    print(f"\n  Time: {elapsed:.1f} min")

    # ── VISUALISATIONS (IMPROVEMENT E) ─────────────────────────────────────────
    # Equity curves, drawdown and trades-per-year charts are saved as PNG files
    # using the non-interactive Agg backend — no display or GUI required.
    # Saved files:
    #   equity_curve_<period_name>.png   (one per period)
    #   drawdown_curve.png               (P1 reference period)
    #   trades_per_year.png              (bar chart across all periods)
    # TODO R1 (V31): add Sharpe ratio subplot and rolling 30-day win-rate panel
    #   to quickly spot where regime changes caused model divergence.
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        for r in results:
            ...
            if eq:
                ...
                plt.plot(dates, values, label=f"Equity {r['name']}")
                plt.title(f"Equity Curve - {r['name']}")
                plt.legend()
                plt.tight_layout()
                plt.savefig(f"equity_curve_{r['name'].replace(' ', '_')}.png")
                plt.close()
        # Drawdown (first period as reference)
        if results and results[0]["equity"]:
            eq_s = pd.Series(
                [e[1] for e in results[0]["equity"]],
                index=[e[0] for e in results[0]["equity"]]
            )
            dd_s = (eq_s / eq_s.cummax() - 1)
            plt.figure(figsize=(10, 4))
            plt.plot(dd_s.index, dd_s.values, color="red", label="Drawdown")
        ...
        if all_trade_list:
            ...
            plt.title("Trades por anio")
            plt.tight_layout()
            plt.savefig("trades_per_year.png")
            plt.close()
        print("  Graficos guardados: equity_curve_*.png  drawdown_curve.png  trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel 
    fname = f"sim_v30_{now}.xlsx"
    try:
