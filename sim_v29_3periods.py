#!/usr/bin/env python
"""
DeepQuant V29 — MAX_POS=1 + SIZE75% + RISK7% - Recover V22b returns with fixed capital
=========================================================================================
V29 HYPOTHESIS: V28b's +12.8% vs V22b's +26.09% gap caused by MAX_POS=2 dilution.
With MAX_POS=1 (concentrated like V22b) + SIZE75% ($6,000 max) + fixed period_capital
base, we should recover V22b-level returns while keeping structural improvements.
...
# === HEADER ===
print("=" * 80)
print("  DEEPQUANT V29 — MAX_POS=1 + SIZE75% + RISK7% | Recover V22b returns (V28b base)")
print(f"  RISK_GUARD={RISK_GUARD_PCT*100:.0f}% (V28b=5%) | PROB_SIZE={PROB_SIZE_MIN*100:.0f}-{PROB_SIZE_MAX*100:.0f}% (V28b=25-55%) | MAX_POS={MAX_POS} (V28b=2)")
print(f"  On $8k capital: max loss/trade=${8000*RISK_GUARD_PCT:.0f} | max single pos=${8000*PROB_SIZE_MAX:.0f} | 1 concentrated slot in BULL windows")
print("=" * 80)
...
if __name__ == "__main__":
    ...
    print(f"\n\n{'='*80}")
    print(f"  SUMMARY - DEEPQUANT V29 (V28b base: MAX_POS=1 + SIZE75% + RISK7%)")
    print(f"{'='*80}")
    ...
    print(f"  V29  {p1['roi']:>+7.2f}% {p2['roi']:>+7.2f}% {p3['roi']:>+7.2f}% {cum_roi:>+7.2f}% {yr_all['2022']:>7s} {yr_all['2023']:>7s} {yr_all['2024']:>7s} {yr_all['2025']:>7s} {yr_all['2026']:>7s}  {passed}/{passed+failed} RISK7/SIZE75/POS1")
    print(f"{'='*90}")

    print(f"\n  Time: {elapsed:.1f} min")

    fname = f"sim_v29_{now}.xlsx"
    try:
