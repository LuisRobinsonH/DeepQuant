#!/usr/bin/env python
"""
DeepQuant V33 — BULL gc_age >= 30 minimum (single-variable vs V32)
==================================================================
V33 HYPOTHESIS: Years with < 10% ROI are caused by 2 failure modes:
...
BULL_GC_MIN_DAYS    = 30        # V33: BULL also requires established GC >= 30d
                                # prevents trading in newborn/unreliable rallies
...
# === HEADER ===
print("=" * 80)
print("  DEEPQUANT V33 — BULL gc_age>=30 | Walk-Forward 2010-2026 | 17 Annual Periods")
...
def calc_tier_v15(date):
    ...
    if gate_3m and vs["golden"] and vs["slope"] and vs["dist_200"] >= 0.02:
        ...
        # V33: require established GC before entering BULL (same logic as MODERATE)
        if vs["gc_age"] >= BULL_GC_MIN_DAYS:
            ...
            detail["reason"] = f"BULL GC too young ({vs['gc_age']}d < {BULL_GC_MIN_DAYS}d)"
            # fall through to MODERATE/BEAR — young GC is not reliable enough for trades
...
if __name__ == "__main__":
    ...
    #  Per-period summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY - DEEPQUANT V33 | BULL gc_age>=30 | Walk-Forward 2010-2026 | 17 Annual Periods")
    print(f"{'='*90}")
    ...
    print(f"")
    print(f"  V32   +169.95% 15/17  gc_age=0 (newborn GC allowed)")
    print(f"  V33 (this run): 17-year walk-forward, BULL gc_age>=30")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("v33_annual_roi.png")
        plt.close()
        ...
        plt.ylabel("Capital ($)")
        plt.tight_layout()
        plt.savefig("v33_equity_compound.png")
        plt.close()
        ...
        if all_trade_list:
            ...
            plt.tight_layout()
            plt.savefig("v33_trades_per_year.png")
            plt.close()

        print("  Graficos guardados: v33_annual_roi.png  v33_equity_compound.png  v33_trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel
    fname = f"sim_v33_{now}.xlsx"
    try:
