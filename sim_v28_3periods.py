...
PROB_SIZE_MIN        = 0.20   # V28: 20% of capital at min prob (was 0.30)  -> $1,600 min position
PROB_SIZE_MAX        = 0.40   # V28: 40% of capital at prob=1.00 (was 0.90) -> $3,200 max position
# RATIONALE: On $8,000 capital, 90% single position = $7,200. 2.5xATR stop at 3% ATR
# = $540 loss per trade (6.8% of capital). At 40% cap, max position=$3,200 → max loss=$196.
RISK_GUARD_PCT       = 0.03   # V28: hard ceiling: actual loss at stop <= 3% of capital (was 10%)
# 3% x $8,000 = $240 max loss per trade. Institutional standard for swing trading.
MIN_POSITION         = 1_500  # V28: lowered from $2k so second slot can open when cash partially used
...
# === HEADER ===
print("=" * 80)
print("  DEEPQUANT V28 — Risk-Correct Sizing + MAX_POS=2 (V22b base)")
...
if __name__ == "__main__":
    ...
    print(f"\n\n{'='*80}")
    print(f"  SUMMARY - DEEPQUANT V28 (V22b + RISK3% + SIZE40% + POS2)")
    print(f"{'='*80}")
    ...
    print(f"  V27  -18.86%   -6.43%   -4.97%  -27.85%   $-413   $-212   $-883   $-515   $-398  0/5 STOCK_BULL+MOD72 FAILED")
    print(f"  V28 {p1['roi']:>+7.2f}% {p2['roi']:>+7.2f}% {p3['roi']:>+7.2f}% {cum_roi:>+7.2f}% {yr_all['2022']:>7s} {yr_all['2023']:>7s} {yr_all['2024']:>7s} {yr_all['2025']:>7s} {yr_all['2026']:>7s}  {passed}/{passed+failed} RISK3/SIZE40/POS2")
    print(f"{'='*90}")

    print(f"\n  Time: {elapsed:.1f} min")

    fname = f"sim_v28_{now}.xlsx"
    try:
