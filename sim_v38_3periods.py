#!/usr/bin/env python
"""
DeepQuant V38 — GC_AGE_MIN=45 + PROB_SIZE_MAX=0.90 (V37 + una sola mejora)
=======================================================================
V38 CHANGES vs V37 (UN SOLO CAMBIO):
  GC_AGE_MIN = 45: El Golden Cross debe tener al menos 45 días de antigüedad
  para permitir entradas BULL. En V37, 2016-Jun el GC era reciente (30d)
  y NHF entró el Jun 1 ? stop Jun 15 (-$444). Con gc_age>=45, todo Junio
  2016 queda clasificado BEAR (gc_age=30 al inicio del mes < 45).
  Resultado esperado: 2016 = 0 trades = PASS. Score 17/17.

  COSTO ESPERADO:
  - 2012: CQR(Apr,gc_age=16) y NAB(Sep,gc_age=0) bloqueados.
    Solo BPT(Oct,gc_age=46) permanece (+$200 vs +$591). 2012 sigue PASS.
  - 2015: CPU(Mar,gc_age=25) bloqueado ? 2015=+$0, sigue PASS.
  - Todos los otros años: gc_age>>45, sin cambio.

  NO cambia training. PROB_SIZE_MAX=0.90 se mantiene de V37.
...
GC_AGE_MIN          = 45      # V38: BULL gate requires GC >= 45 days old (blocks fresh GC entries)
...
def calc_tier_v15(date):
    ...
    if gate_3m and vs["golden"] and vs["slope"] and vs["dist_200"] >= 0.02 and vs["gc_age"] >= GC_AGE_MIN:
...
if __name__ == "__main__":
    ...
    #  Per-period summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY - DEEPQUANT V38 | GC_AGE_MIN={GC_AGE_MIN} | PROB_SIZE_MAX=0.90 | Walk-Forward 2010-2026")
    print(f"{'='*90}")
    ...
    print(f"  V36   +209.72% 16/17  STOP_COOLDOWN=5")
    print(f"  V37   +290.84% 16/17  PROB_SIZE_MAX=0.90 | STOP_COOLDOWN=5")
    print(f"  V38 (this run): GC_AGE_MIN={GC_AGE_MIN} | PROB_SIZE_MAX=0.90 | STOP_COOLDOWN=5")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("v38_annual_roi.png")
        plt.close()
        ...
        plt.ylabel("Capital ($)")
        plt.tight_layout()
        plt.savefig("v38_equity_compound.png")
        plt.close()
        ...
        if all_trade_list:
            ...
            plt.tight_layout()
            plt.savefig("v38_trades_per_year.png")
            plt.close()

        print("  Graficos guardados: v38_annual_roi.png  v38_equity_compound.png  v38_trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel
    fname = f"sim_v38_{now}.xlsx"
    try:
