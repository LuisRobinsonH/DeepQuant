#!/usr/bin/env python
"""
DeepQuant V36 — STOP_COOLDOWN_5 solo (V32 + una sola mejora)
=================================================================
V36 CHANGES vs V32 (UN SOLO CAMBIO, sin tocar training):
  STOP_COOLDOWN_DAYS=5: Tras un exit STOP o EMERGENCY, pausar entradas
  BULL durante 5 días calendario.
...
if __name__ == "__main__":
    ...
    #  Per-period summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY - DEEPQUANT V36 | STOP_COOLDOWN=5 | Walk-Forward 2010-2026")
    print(f"{'='*90}")
    ...
    print(f"  V35   +078.40% 14/17  SL=2.0+cooldown FAILED (2013 broke)       ")
    print(f"  V36 (this run): STOP_COOLDOWN=5d only (no SL change)            ")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("v36_annual_roi.png")
        plt.close()
        ...
        plt.ylabel("Capital ($)")
        plt.tight_layout()
        plt.savefig("v36_equity_compound.png")
        plt.close()
        ...
        if all_trade_list:
            ...
            plt.tight_layout()
            plt.savefig("v36_trades_per_year.png")
            plt.close()

        print("  Graficos guardados: v36_annual_roi.png  v36_equity_compound.png  v36_trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel
    fname = f"sim_v36_{now}.xlsx"
    try:
