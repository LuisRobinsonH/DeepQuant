#!/usr/bin/env python
"""
DeepQuant V13 — Best of V11 + V12 Combined
=============================================
ROOT CAUSE OF V12/V12.1 FAILURE:
  Target "4%+ in 20d with <3.5% DD" has ~15-20% positive rate
  → Calibrated probabilities cluster at 0.15-0.25
  → Threshold of 0.48 blocks 99% of signals → 0-3 trades total!

V13 SOLUTION: Combine V11's ALIGNED TARGET with V12's 3-MONTH GATE

ALIGNED TARGET (V11): "Reach +1.5xATR BEFORE -2.5xATR in 25d"
  → 65% positive rate → probabilities center around 0.55-0.65
  → Directly aligned with our trading stops (BE at +1.5xATR, SL at -2.5xATR)

3-MONTH VAS GATE (V12): Proven to eliminate ALL 2022 trades
  → Requires 3 consecutive positive VAS monthly returns
  → Also requires Golden Cross + SMA50 slope + VAS>SMA200 by 2%

STOCK FILTERS: Relaxed (let ML + regime do the work)
  → Price > SMA50, Price > SMA20, RSI 30-80, mom5 > -1%
  → No breakout filter, no commission edge check

RISK: Daily stops (2-day grace), 1 position, $6400
  → Stop 2.5xATR, BE trigger +1.5xATR, trail 1.5xATR
  → YTD cap -$600, max 12 trades/year, max 3/month
  → No re-entry on same ticker within 10 days of exit
"""
...
MAX_TRADES_MONTH = 3
MAX_TICKER_TRADES = 3
TICKER_COOLDOWN = 10            # days before re-entering same ticker
PROB_FULL       = 0.52          # for aligned target, this is above base rate
PROB_SELECTIVE  = 0.58
YTD_LOSS_CAP   = -600
LS_MAX = 3
LS_DAYS = 7
GRACE_DAYS = 2                  # no stop check in first 2 days
...
def build_aligned_target(df, be_atr=1.5, sl_atr=2.5, max_days=25):
    ...
    Base rate ~62.5% (from random walk: sl/(be+sl) = 2.5/4.0).
    If model can push to 65%+, there is genuine alpha."""
    c = df["Close"]
    h = df["High"]
    l = df["Low"]
    atr = volatility.average_true_range(h, l, c, 14)
...
print("=" * 70)
print("  DEEPQUANT V13 - Best of V11 (aligned target) + V12 (3-month gate)")
print("  Aligned Target | 3-Month Gate | Relaxed Filters | 1 Position")
print("=" * 70)
print(f"  ALIGNED TARGET: reach +{BE_TRIGGER_ATR}xATR before -{SL_ATR}xATR in 25d")
print(f"  Base rate ~{SL_ATR/(BE_TRIGGER_ATR+SL_ATR)*100:.1f}% (random walk)")
print(f"  Prob threshold: Full >= {PROB_FULL}, Selective >= {PROB_SELECTIVE}")
print(f"  3-Month VAS Gate + Golden Cross + SMA50 Slope + VAS>SMA200+2%")
print(f"  Stop: {SL_ATR}xATR | BE: +{BE_TRIGGER_ATR}xATR | Trail: {TRAIL_ATR}xATR")
...
def simulate_period(name, start, end, train_end, capital):
    ...
    # Training with ALIGNED target
    print(f"  Training (ALIGNED target: +{BE_TRIGGER_ATR}xATR before -{SL_ATR}xATR in 25d)...")
    models = {}
    target_stats = []
    ...
    print(f"\n   Models: {len(models)}/{len(valid)} | Avg target positive rate: {avg_pos_rate:.1f}%")
    ...
    monthly_trades = {}
    ticker_last_exit = {}  # cooldown tracking
    ...
    # Debug: count candidates passing each filter per month
    filter_debug = {}
    ...
    for day in trading_days:
        ...
        if position is not None:
            ...
            if day in data[t].index:
                ...
                # Breakeven trigger (checked daily)
                ...
                # Regime lost + profitable => protect gains
        ...
        for t in models:
            ...
            if t in ticker_last_exit:
                days_since = (day - ticker_last_exit[t]).days
                if days_since < TICKER_COOLDOWN:
                    continue
        ...
        # Log filter pass rates for first day of each month
        if month not in filter_debug:
            filter_debug[month] = {
                "regime": gates_ok, "score": score,
    ...
    for m in sorted(filter_debug.keys()):
        if filter_debug[m]["regime"]:
            d = filter_debug[m]
            print(f"    {m[0]}-{m[1]:02d}: score={d['score']} SMA:{d['sma']} RSI+:{d['rsi']} Prob+:{d['prob']} Cands:{d['candidates']}")
    ...
    if n > 0:
        ...
        for tr in trades:
            ...
            print(f"       [{status}] {tr['ticker']:8s} {str(tr['entry'].date()):10s}->{str(tr['exit'].date()):10s} {tr['days']:3d}d prob:{tr['prob']:.2f} ${tr['pnl']:>+8,.2f} ({tr['reason']})")
...
if __name__ == "__main__":
    ...
    print(f"  V13 {p1['roi']:>+7.2f}% {p2['roi']:>+7.2f}% {p3['roi']:>+7.2f}% {cum_roi:>+7.2f}% {yr_all['2022']:>7s} {yr_all['2023']:>7s} {yr_all['2024']:>7s} {yr_all['2025']:>7s} {yr_all['2026']:>7s}")
    print(f"{'='*90}")

    print(f"\n  Time: {elapsed:.1f} min")

    fname = f"sim_v13_{now}.xlsx"
    try:
