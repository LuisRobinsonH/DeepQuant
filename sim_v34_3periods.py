#!/usr/bin/env python
"""
DeepQuant V34 — gate_6m for BULL (single variable vs V32)
============================================================
V34 HYPOTHESIS: 2016 and 2019 FAIL years had macro shocks (China/Oil crash,
US-China trade war) that hit AFTER a well-established BULL. The gate_3m check
passed because Oct/Nov/Dec 2015 and mid-2019 had partially recovered, masking
the underlying macro risk.

V33 LESSON (FAILED): gc_age>=30 made 2012 WORSE. The 3 winning trades in
April 2012 had gc_age=16 (blocked by V33) and the 2 trades allowed at gc_age=30
were losers. WRONG HYPOTHESIS. V34 reverts the gc_age BULL change.

V34 CHANGE (single variable): Extend BULL gate from gate_3m to gate_6m.
  - gate_3m: last 3 VAS months must be positive
  - gate_6m: last 6 VAS months must be positive (half a year of momentum)

WHY gate_6m SHOULD FIX 2016 AND 2019:
  2016: August 2015 VAS was -8.4% (China crash). gate_6m (Aug-Jan) fails.
        Under gate_3m, only Oct-Dec 2015 was checked (all recovered -> PASS).
  2019: Q4 2018 global selloff. gate_6m would include negative late-2018 months.

RISK: May block early-recovery years (2013, 2021) if the prior 6 months
include the tail end of a bad period. Monitor trade count in good years.

V32 CHANGES vs V31:
  A) THIN BULL WINDOW: Only 22-55 BULL trading days in the year.
     A newborn Golden Cross (gc_age < 30) is unreliable — ASX 2015
     had a 25-day GC that collapsed in August. We entered with 2 small
     trades and barely cleared commissions ($207 = +2.58%).
  B) LOSS DRAG: Enough BULL days but 1-2 losing trades pull a good year
     below 10% (2024: 6 winners offset by 2 × -$440 losses).

V33 CHANGE (single variable): Add BULL_GC_MIN_DAYS = 30 to BULL gate.
  Currently BULL gate requires gc_age >= 0 (no minimum).
  MODERATE already requires gc_age >= 30 (MOD_GC_MIN_DAYS).
  V33 applies the same logic to BULL: don't trade until the Golden
  Cross has been confirmed for >= 30 consecutive days.

EXPECTED IMPACT:
  2015: gc_age=25 in March → SKIP BULL → 0 trades → treat as bank year
  2012 April: gc_age=16 → SKIP → wait for Oct (gc_age=46) → trade then
  2019: depends on gc_age at entry — may eliminate early false-positives
  2021, 2013, 2018: gc_age established → no change
...
np.random.seed(42)
random.seed(42)
DATA_CACHE = Path("sim_v32_data_cache.pkl")  # V33: reuse V32 cache (same data)
...
PROB_SIZE_MAX        = 0.75   # V29: 75% of capital at prob=1.00 -> $6,000 max position
# RATIONALE: At $6,000 pos: $39.90 roundtrip = 0.67% drag (very efficient).
# V28b's 0.55 max ($4,400) diluted gains; V22b's 0.90 ($7,200) allowed $341 losses.
# V29's 0.75 ($6,000) is the sweet spot: larger gains + CommSec efficient + bounded risk.
...
# === HEADER ===
print("=" * 80)
print("  DEEPQUANT V34 — BULL gate_6m | Walk-Forward 2010-2026 | 17 Annual Periods")
...
def calc_tier_v15(date):
    ...
    # === BULL GATE (V34: gate_6m instead of gate_3m) ===
    prior_6 = get_prior_months(date, 6)
    gate_6m = prior_6 is not None and all(r > 0 for r in prior_6)
    detail["gate_6m"] = gate_6m
    if prior_6:
        detail["months6"] = [f"{r*100:+.1f}%" for r in prior_6]

    if gate_6m and vs["golden"] and vs["slope"] and vs["dist_200"] >= 0.02:
    ...
    # === MODERATE GATE (V16c+V34: 2-month gate) ===
...
if __name__ == "__main__":
    ...
    #  Per-period summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY - DEEPQUANT V34 | BULL gate_6m | Walk-Forward 2010-2026 | 17 Annual Periods")
    print(f"{'='*90}")
    ...
    print(f"")
    print(f"  V32   +169.95% 15/17  gate_3m (baseline)")
    print(f"  V33   +141.41% 14/17  gc_age>=30 FAILED (made 2012 worse)")
    print(f"  V34 (this run): 17-year walk-forward, BULL gate_6m")
    ...
    try:
        ...
        plt.title("V32 — Annual ROI by Year (2010-2026)")
        plt.ylabel("ROI (%)")
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig("v34_annual_roi.png")
        plt.close()
        ...
        plt.title(f"V32 — Compounded Equity 2010-2026 (Final: ${running:,.0f})")
        plt.ylabel("Capital ($)")
        plt.tight_layout()
        plt.savefig("v34_equity_compound.png")
        plt.close()
        ...
        if all_trade_list:
            ...
            plt.title("V32 — Trades por Año")
            plt.tight_layout()
            plt.savefig("v34_trades_per_year.png")
            plt.close()

        print("  Graficos guardados: v34_annual_roi.png  v34_equity_compound.png  v34_trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel
    fname = f"sim_v34_{now}.xlsx"
    try:
