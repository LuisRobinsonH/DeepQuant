#!/usr/bin/env python
"""
DeepQuant V35 — SL 2.0 + STOP_COOLDOWN_5 (contra black swans 2016/2019)
=========================================================================
V35 CHANGES vs V32 (two targeted changes, rest identical):
  1. SL_ATR 2.5 → 2.0: Stop loss tigher. Dos efectos:
     a) Cada trade perdedor pierde ~20% menos (e.g. FMG 2019 -$461 → ~-$370).
     b) Target de entrenamiento también usa 2.0x — modelo aprende con el
        mismo stop que se ejecutará en simulación (alineación total).
  2. STOP_COOLDOWN_DAYS=5: Tras un exit STOP o EMERGENCY, pausar entradas
     BULL durante 5 días calendario. Objetivo:
     - 2016: NHF.AX para el 15 Jun (STOP) → bloquea WOR.AX (mismo día)
       Ahorra -$369 de WOR. Net 2016: -$382→ sigue en FAIL pero reducido.
     - 2019: FMG.AX para el 3 Jun (STOP) → bloquea LYC.AX (mismo día)
       LYC fue -$552 en 1 día (China rare earth shock). Sin LYC:
       2019 = SUN(+$98) + FMG(~-$370) + APA(+$447) - comm ≈ +$55 → PASS!

DIAGNÓSTICO (diagnose_v35.py sobre datos reales):
  2016-Jun: gate_3m PASS, gc_age=30, bull_score=100 — BREXIT 23 Jun impredecible
  2019-Apr: gate_3m PASS, gc_age=21 → SUN (WIN)
  2019-May: gate_3m PASS, gc_age=73 → FMG (STOP, trade war re-escalation)
  2019-Jun: gate_3m PASS, gc_age=84 → LYC (1-day EMERGENCY, China rare earth)
  CONCLUSIÓN: Ningún filtro de régimen detecta los black swans. La mejora
  tiene que ser en gestión de posición (stop tighter + cooldown post-stop).

FALLOS PREVIOS (V33/V34):
  V33: gc_age>=30 para BULL → 14/17 FAIL (bloqueó winners 2012 con gc_age=16)
  V34: gate_6m para BULL → 15/17 mismo score PERO 0 trades 2010-2016 (muy restrictivo)
...
SL_ATR              = 2.0     # V35: 2.5→2.0 tighter stop (FMG 2019 loss ~20% lower, aligned with training)
BE_TRIGGER_ATR      = 1.5
TRAIL_ATR           = 1.5
MAX_HOLD            = 35   # V22 test confirmed: shorter hold (20d) exposed to lower-quality sequential signals
GRACE_DAYS          = 2
STOP_COOLDOWN_DAYS  = 5       # V35: pause BULL entries N days after STOP/EMERGENCY exit
...
def simulate_period(name, start, end, train_end, capital):
    ...
    # V35: Stop cooldown tracker
    last_stop_date      = None    # set when STOP/EMERGENCY fires; blocks BULL entries for STOP_COOLDOWN_DAYS
    ...
    for day in trading_days:
        ...
        for pos in list(positions):
            ...
            if day in data[t].index:
                ...
                if exit_reason:
                    ...
                    # V35: Record stop date for cooldown
        ...
        if entry_mode == "BULL":
            ...
            if bull_trades_month.get(month, 0) >= BULL_MAX_MONTH:
                continue
            # V35: Stop cooldown — skip BULL entries within STOP_COOLDOWN_DAYS after a STOP/EMERGENCY
...
if __name__ == "__main__":
    ...
    #  Per-period summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY - DEEPQUANT V35 | SL_ATR=2.0 + STOP_COOLDOWN=5 | Walk-Forward 2010-2026")
    print(f"{'='*90}")
    ...
    print(f"")
    print(f"  V32   +169.95% 15/17  gate_3m SL=2.5 (baseline)")
    print(f"  V33   +141.41% 14/17  gc_age>=30 FAILED")
    print(f"  V34   +015.49% 15/17  gate_6m FAILED (0 trades 2010-2016)")
    print(f"  V35 (this run): SL_ATR=2.0 + STOP_COOLDOWN=5 days")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("v35_annual_roi.png")
        plt.close()
        ...
        plt.ylabel("Capital ($)")
        plt.tight_layout()
        plt.savefig("v35_equity_compound.png")
        plt.close()
        ...
        if all_trade_list:
            ...
            plt.tight_layout()
            plt.savefig("v35_trades_per_year.png")
            plt.close()

        print("  Graficos guardados: v35_annual_roi.png  v35_equity_compound.png  v35_trades_per_year.png")
    except Exception as e:
        print(f"  Warning graficos: {e}")

    #  Export to Excel
    fname = f"sim_v35_{now}.xlsx"
    try:
